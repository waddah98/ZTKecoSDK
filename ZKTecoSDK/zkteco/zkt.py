...
class ZK(object):
    ...
    def __init__(self, ip='192.168.1.221', port=4370, timeout=20, password=0, force_udp=False, ommit_ping=False, verbose=False,
                 encoding='UTF-8'):
        """
    ...
    def __send_command(self, command, command_string=b'', response_size=8):
        ...
        if command not in [CMD_CONNECT, CMD_AUTH] and not self.is_connect:
            raise Exception("instance are not connected.")
        ...
        try:
            if self.tcp:
                ...
                if self.__tcp_length == 0:
                    raise Exception("TCP packet invalid")
    ...
    def connect(self):
        ...
        if not self.ommit_ping and not self.helper.test_ping():
            raise Exception("can't reach device (ping %s)" % self.__address[0])
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("Invalid response: Can't connect")
    ...
    def enable_device(self):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("Can't enable device")
    ...
    def disable_device(self):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("Can't disable device")
    ...
    def get_serialnumber(self):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("Can't read serial number")
    ...
    def get_mac(self):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("can't read mac address")
    ...
    def free_data(self):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("can't free data")
    ...
    def read_sizes(self):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("can't read sizes")
    ...
    def restart(self):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("can't restart device")
    ...
    def write_lcd(self, line_number, text):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("can't write lcd")
    ...
    def clear_lcd(self):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("can't clear lcd")
    ...
    def get_time(self):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("can't get time")
    ...
    def set_time(self, timestamp):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("can't set time")
    ...
    def poweroff(self):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("can't poweroff")
    ...
    def refresh_data(self):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("can't refresh data")
    ...
    def set_user(self, uid=None, name='', privilege=0, password='', group_id='', user_id='', card=0):
        ...
        :param privilege: check the py for reference
        ...
        if self.user_packet_size == 28:
            ...
            try:
                ...
                raise Exception("Can't pack user")
        else:
        ...
        if not cmd_response.get('status'):
            raise Exception("Can't set user")
        self.refresh_data()
    ...
    def _send_with_buffer(self, buffer):
        ...
        cmd_response = self.__send_command(command, command_string)
        if not cmd_response.get('status'):
            raise Exception("Can't prepare data")
    ...
    def __send_chunk(self, command_string):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("Can't send chunk")
    ...
    def get_users_by_id(self, id):
        ...
        for x in range(len_users):
            if users[x].user_id == str(id):
                found_user.append(users[x])
        return found_user
    ...
    def reg_event(self, flags):
        ...
        cmd_response = self.__send_command(command, command_string)
        if not cmd_response.get('status'):
            raise Exception("cant' reg events %i" % flags)
    ...
    def set_sdk_build_1(self):
        ...
        def enroll_user(self, uid=0, temp_id=0, user_id=''):
            ...
            cmd_response = self.__send_command(command, command_string)
            if not cmd_response.get('status'):
                raise Exception("Cant Enroll user #%i [%i]" % (uid, temp_id))
    ...
    def clear_data(self):
        ...
        if cmd_response.get('status'):
            ...
            raise Exception("can't clear data")
    ...
    def __read_chunk(self, start, size):
        ...
        for _retries in range(3):
            ...
            raise Exception("can't read chunk %i:[%i]" % (start, size))
    ...
    def read_with_buffer(self, command, fct=0, ext=0):
        ...
        data = []
        start = 0
        cmd_response = self.__send_command(_CMD_PREPARE_BUFFER, command_string, response_size)
        if not cmd_response.get('status'):
            raise Exception("RWB Not supported")
